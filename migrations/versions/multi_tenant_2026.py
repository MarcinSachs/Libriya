...
def upgrade():
    ...
    conn = op.get_bind()
    # Different SQL dialects have different "ignore if exists" syntax
    if conn.dialect.name == 'sqlite':
        op.execute("INSERT OR IGNORE INTO tenant (id, name) VALUES (1, 'default')")
    else:
        # MySQL/MariaDB: use INSERT IGNORE
        op.execute("INSERT IGNORE INTO tenant (id, name) VALUES (1, 'default')")
